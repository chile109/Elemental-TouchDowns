...
public class ARController : MonoBehaviour
{
    ...
    // The GameObject which holds the MeshFilter and MeshRenderer for the background video, and also the Camera object(s) used to render them. 
    private GameObject _videoBackgroundMeshGO0 = null;

    // An array used to fetch pixels from the native side, only if not using native GL texturing.
    private Color32[] _videoColor32Array0 = null;

    // Texture object with the video image.
    private Texture2D _videoTexture0 = null;

    // Material which uses our "VideoPlaneNoLight" shader, and paints itself with _videoTexture0.
    ...
    void Start()
    {
        ...
        {
            EditorUtility.DisplayDialog("ARToolKit6: Build target not supported", "The selected build target is not supported by ARToolKit6.\nARToolKit supports macOS, Linux, iOS and Android. Please change the build target back to one of these options.", "OK");
        }
#endif
    ...
    public bool StartAR()
    {
        ...
        // Load the default camera parameters. 讀取相機設定
        byte[] cparam0 = null;
        byte[] cparam1 = null;
        byte[] transL2R = null;
        ...
        // Begin video capture and marker detection. 開始偵測畫面中marker
        if (!VideoIsStereo)
        {
        ...
        // Remaining Unity setup happens in UpdateAR(). 保持初始化在editor前
        return true;
    }
    ...
    public void SetContentForScreenOrientation(bool cameraIsFrontFacing)
    {
        ...
        { // Landscape with top of device at left.
        ...
        { // Landscape with top of device at right.
            ContentRotate90 = false;
            ContentFlipV = true;
            ContentFlipH = (!cameraIsFrontFacing);
        }
